...
export function TemplateSelect() {
  ...
  const { data, isLoading, refetch } = useMessageTemplates(true)
  ...
  if (isLoading || !data) {
    return <p className="text-zinc-600">Carregando templates...</p>
  }
    ...
    return (
      <>
        <p className="text-red-800">
          Erro ao carregar templates: {data?.error || "Erro desconhecido"}
        </p>
        <p className="text-zinc-800">
          <button type="button" onClick={refetch}>
            <strong className="cursor-pointer font-medium text-primary-800 underline hover:text-primary-700">
              Clique aqui
            </strong>
          </button>{" "}
